...
"
function! s:get_html_color()
    let iskeyword_saved = &iskeyword
    set iskeyword=48-57,65-70,97-102,#
    let w = expand("<cword>")
    let &iskeyword = iskeyword_saved

	if w =~ '^#\([a-fA-F0-9]\{3\}\|[a-fA-F0-9]\{6\}\)$'
        norm F#
        if strlen(w)==4
            norm d4lh
        else
...
nmap <f5> :ColorPicker<Cr>

command ColorPicker call ColorPicker()

function! ColorPicker()
let color = s:get_html_color()
python << EOF
import pygtk
pygtk.require('2.0')
import gtk
import vim
color=gtk.gdk.Color(vim.eval("color"))
color_sel = gtk.ColorSelectionDialog("VIM Color Picker")
color_sel.colorsel.set_current_color(color)
