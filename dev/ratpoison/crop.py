...
try:
    ...
    errorDialog("Este script necesita gtk, pygtk 2.8 o superior, pycairo y Python Image Library (PIL)")
except AssertionError:
    errorDialog("Este script necesita pygtk 2.8 o superior")
...
class CropWidget(gtk.DrawingArea):
    ...
    def move(self, widget,event):
        self.px = max(0,min(event.x,self.iw))
        self.py = max(0,min(event.y,self.ih))
        self.queue_draw()
...
class cropWin():
    def askforfile(self,wtf=0):
        ...
        fc = gtk.FileChooserDialog("Selecciona donde guardar la imagen recortada",
        ...
        if res == gtk.RESPONSE_OK:
            ...
            self.fs_button.set_label("Seleccionar")
    ...
    def save(self,*wtf):
        i = self.cw
        if not i.havep0:
            errorDialog("Selecciona un area de la imagen haciendo clic y arrastrando")
            return
        pixbuf = i.pixbuf
        ...
        print((x1,x2),(y1,y2))
        print (min(x1,x2),min(y1,y1),max(x1,x2),max(y1,y2))
    ...
    def __init__(self,file):
        ...
        save_frame = gtk.Frame("Guardar en")
        save_btn = gtk.Button("Guardar")
        cancel_btn = gtk.Button("Cancelar")
        ...
        r1 = self.r1 = gtk.RadioButton(None,"Remplazar la imagen original")
        r2 = self.r2 = gtk.RadioButton(r1,"Guardar como...")
        ...
        format_frame = gtk.Frame("Formato")
        format_frame.add(format_vbox)


        fs_button = self.fs_button = gtk.Button("Seleccionar")
        ...
        mainvbox.pack_start(gtk.Label("Haz click y arrastra para seleccionar el area a recortar"),False)
